...
result.lss <- mclapply(shop.ls, function(shop.id) {
    ...
    tot.wks <- difftime(ed.dt, start.date, units = "weeks") %>% floor(.)
    if (which(shop.id==dt.dly.stats$ShopId) %% 100==0) {
        # cat(sprintf("%s\n", as.character(start.date)))
        cat(sprintf("shop:%s dur:%s st.dt:%s\n", shop.id, as.character(start.date)))
    }
    per <- "week"
