...
fls <- list.files("data/processed/%s", ".RData", full.names = TRUE)
dtb <- NULL
for (f in fls) {
    load(f)
...
dt1 <- dt[, .(ShopId, observed, trend, diff)]
dt1[, `:=` (cum.diff.percentage=round(cumsum(diff)*100/sum(diff), 1),
            percentage=round(diff*100/sum(diff), 1),
            percentage2=round(diff*100/observed, 1))]
plot.ecdf(dt1$diff)
dt1[, sum(diff)]
