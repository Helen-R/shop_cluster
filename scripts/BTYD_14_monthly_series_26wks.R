...
load(fnm)
date.ls <- seq.Date(as.Date("2013-07-01"), as.Date("2017-04-30"), "month")

result.lss <- lapply(date.ls, function(start.date) {
param.ls <- NULL
result.ls <- NULL
...
end.date <- start.date %m+% weeks(52)
...
cut.date <- start.date %m+% weeks(26)
...
png(sprintf("plots/plot7_%s_%s.png", shop.id, as.character(start.date)))
bgnbd.PlotFrequencyInCalibration(params, cal.cbs, censor)
dev.off()
png(sprintf("plots/plot8_%s_%s.png", shop.id, as.character(start.date)))
bgnbd.PlotTransactionRateHeterogeneity(params)
dev.off()
png(sprintf("plots/plot9_%s_%s.png", shop.id, as.character(start.date)))
bgnbd.PlotDropoutRateHeterogeneity(params)
dev.off()

png(sprintf("plots/plot60_%s_%s.png", shop.id, as.character(start.date)))
...
png(sprintf("plots/plot5_%s_%s.png", shop.id, as.character(start.date)))
...
png(sprintf("plots/plot6_%s_%s.png", shop.id, as.character(start.date)))
...
# #         t.x = 20, T.cal = 39)
# #     cat ("x:",i,"\t Expectation:",cond.expectation, fill = TRUE) }
# png(sprintf("plots/plot7-1_%s_%s.png", shop.id, as.character(start.date)))
# pnbd.PlotFrequencyInCalibration(params, cbs, censor)
# dev.off()
# png(sprintf("plots/plot8-1_%s_%s.png", shop.id, as.character(start.date)))
# pnbd.PlotTransactionRateHeterogeneity(params)
# dev.off()
# png(sprintf("plots/plot9-1_%s_%s.png", shop.id, as.character(start.date)))
...
# # }
# 
# png(sprintf("plots/plot60-1_%s_%s.png", shop.id, as.character(start.date)))
...
# result.ls <- c(result.ls, list(comp))
# 
# png(sprintf("plots/plot5-1_%s_%s.png", shop.id, as.character(start.date)))
# inc.tracking <- pnbd.PlotTrackingInc(params, T.cal,
#                                      T.tot, w.track.data,
#                                      n.periods.final)
# dev.off()
# png(sprintf("plots/plot6-1_%s_%s.png", shop.id, as.character(start.date)))
...
# png(sprintf("plots/plot61_%s_%s.png", shop.id, as.character(start.date)))
...
system("zip plots plots/*")
gcs_upload("plots.zip")
