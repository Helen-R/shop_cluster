...
shop.id <- 14
fnm <- sprintf("data/intermediates/dt.dly_prod_%s.RData", shop.id)
load(fnm)
setnames(dt.dly, "gorss.sale", "gross.sales")
dt.dly.raw <- dt.dly
date.ls <- seq.Date(as.Date("2013-07-01"), as.Date("2016-04-30"), "month")
suffix <- sprintf("BTYD_%s_monthly_series_cumulate_prod", shop.id)
if (!dir.exists(sprintf("plots/%s/", suffix))) 
    dir.create(sprintf("plots/%s/", suffix))
...
# result.lss <- mclapply(shop.ls, function(shop.id) {
    ...
    ed.dt <- max(dt.dly.raw$order.date)
    # ed.dt <- dt.dly.stats[ShopId==shop.id]$ed.dt
    duration <- as.integer(ed.dt - start.date) + 1
    # duration <- dt.dly.stats[ShopId==shop.id]$duration
    if (duration > 365 * 2) { # data points are more than 2 years --> test period = 52 weeks
        tot.wks <- difftime(ed.dt, start.date, units = "weeks") %>% floor(.) %>% as.integer()
        end.date <- start.date %m+% weeks(tot.wks)
        mid.wks <- 52 # data points less --> test period = half of the total period
    ...
    elog <- dt.dly[, .(SalePageId, order.date, gross.sales)]
    ...
    # holdout period----
    holdout.elog <- elog[date > cut.date & date <=end.date, ]
